...
```{r}
empData2 <- empData[order(names(empData))]
```



```{r}
test <- empData2[['P2_emp_1']]
test2 <- empData2[['P12_emp_1']]
...
colnames(test) == colnames(test2)

test2 %>% 
  select(ifelse(colnames(.) != colnames(test2), 0, 1))
